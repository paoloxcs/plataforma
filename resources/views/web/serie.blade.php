   ...
   {{-- JHED FRONT V2 --}}
    <section class="col-md-4 col-xs-12 col-card-plan">
      {{--<div class="card-suplemento">
        ...
        @if($recents->count()>0)
        <h5 class="text-center font-weight mt-5 mt-2-mob">LAS SERIES MÁS VISTOS</h5>
        <hr>
        @endif
              @foreach($recents as $post)
               <section class="card-video-vis row col-md-12">
                <div class="img col-md-4 col-xs-4">
                  <a href="{{route('getserie',$post->pslug)}}"><img src="{{asset('posts/'.$post->image)}}" width="105%"></a>
                   {{--<span><a href="{{route('getvideo',$post->pslug)}}" class="a-icon"><i class="fas fa-play-circle"></i></a></span>--}}</span>
                </div>
                <div class="col-md-8 col-xs-8">
                  <a href="{{route('getserie',$post->pslug)}}" class="td-none"><p class="title font-weight">{!!substr($post->titulo,0,55)!!}. . . </p></a>
...
<script>
  ...
  function saveMarker(post_id) {
    ...
    $.ajax({
      ...
      success:function(data){
        console.log(data);
        alert('¡Serie marcado como favorito!');
        location.reload();
      },
      error:function(data){
        console.log(data);
