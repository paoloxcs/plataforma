...
@section('content')
  <section class="col-md-12 bg-gris pbm-0 s-cursos"> 
    ...
    <section class="col-md-10 mx-auto row pbm-0 mt-5 mt-2-mob padding-1">
      <h5 class="title-c pbm-0">ARTÍCULOS</h5>
    </section>


    <div class="d-flex align-items-start col-md-10 mx-auto row padding-1">
      {{-- JHED FRONT V2 --}}
      {{-- <div class="nav flex-column nav-pills  col-md-2 mt-1" id="v-pills-tab" role="tablist" aria-orientation="vertical"> --}}
      <div class="nav flex-column nav-pills col-md-3 col-xs-12 filter-articulorubro mt-1" id="v-pills-tab" role="tablist" aria-orientation="vertical">
        ...
        <br>
        <a href="{{route('articuloCat',$categoria->slug)}}" class="nav-link btn-left ">{{$categoria->nombrecategoria}}</a>
       ...
       <hr class="hr-mob-cur" id="curso-vivo">
        @foreach($subcategorias as $scat)
          ...
          <a href="{{route('articuloSCat',$scat->slug)}}" class="nav-link btn-left active " style="width: 100%;position: relative;display: inline-block">{{$scat->nombresubcategoria}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span class="badge badge-secondary float-right" style="background: #24db37;float:right;margin-top: 0px">{{$scat->countArticulos()}}</span></a>
          @else
            <a href="{{route('articuloSCat',$scat->slug)}}" class="nav-link btn-left " style="width: 100%;position: relative;display: inline-block">{{$scat->nombresubcategoria}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span class="badge badge-secondary float-right" style="background: #24db37;float:right;margin-top: 0px">{{$scat->countArticulos()}}</span></a>
          @endif
        @endforeach
