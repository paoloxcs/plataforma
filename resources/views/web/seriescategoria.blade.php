...
@section('titulo','SERIE de '.$categoria->nombrecategoria)
@section('content')
      ...
      <section class="col-md-3 col-xs-12 filter-articulorubro">
        ...
        <ul class="ul-left mt-5 mt-2-mob">
          ...
          @foreach($subcategorias as $scat)
            <li class="">
              <a href="{{route('serieSCat',$scat->slug)}}" class="" style="width: 80%;position: relative;display: inline-block">{{$scat->nombresubcategoria}}</a>
              <span class="badge badge-secondary float-right" style="background: #24db37;float:right;margin-top: 1%">{{$scat->countSeries()}}</span>
            </li>
          @endforeach
        </ul>
