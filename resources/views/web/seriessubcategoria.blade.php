...
@section('titulo','SERIE de '.$subcategoria->nombresubcategoria)
@section('content')
  <section class="col-md-12 bg-gris pbm-0 s-cursos"> 
    ...
    <section class="col-md-10 mx-auto row pbm-0 mt-5 mt-2-mob padding-1">
      <h5 class="title-c pbm-0">SERIE</h5>
    </section>
    ...
    <div class="d-flex align-items-start col-md-10 mx-auto row ">
      ...
      <div class="nav flex-column nav-pills col-md-3 col-xs-12 mt-1 padding-1" id="v-pills-tab" role="tablist" aria-orientation="vertical">
        <h5 class="font-weight"><a href="{{'/series/rubro/'.$categoria->rubro->slug}}" style="color:black;text-decoration: none"> Mostrar Todos </a></h5>
        <br>
        <a href="{{route('serieCat',$categoria->slug)}}" class="nav-link btn-left ">{{$categoria->nombrecategoria}}</a>
      ...
      <hr class="none-mobile">
       <hr class="hr-mob-cur" id="curso-vivo">
        ...
        @foreach($subcategorias as $scat)
          ...
          <a href="{{route('serieSCat',$scat->slug)}}" class="nav-link btn-left active " style="width: 100%;position: relative;display: inline-block">{{$scat->nombresubcategoria}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span class="badge badge-secondary float-right" style="background: #24db37;float:right;margin-top: 0px">{{$scat->countSeries()}}</span>

          </a>

          @else
            <a href="{{route('serieSCat',$scat->slug)}}" class="nav-link btn-left " style="width: 100%;position: relative;display: inline-block">{{$scat->nombresubcategoria}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span class="badge badge-secondary float-right" style="background: #24db37;float:right;margin-top: 0px">{{$scat->countSeries()}}</span>
           </a>
          @endif
        @endforeach
      ...
      {{-- JHED FRONT V2 --}}
      {{-- <div class="tab-content col-md-10 mt-1 mt-5-mob" id="v-pills-tabContent"> --}}
      <div class="tab-content col-md-9 col-xs-12 mt-1 mt-5-mob" id="v-pills-tabContent">
          ...
          <div class="col-md-12 row mt-5 mt-2-mob">
            ...
            @foreach($posts as $post)
               <section class="col-md-4 col-xs-12 mt-2-mob card-curso card-curso-p">
                ...
                <div class="text">
                  <a href="{{route('getserie',$post->pslug)}}" class="td-none"><h4 class="title font-weight">{{$post->titulo}}</h4></a>
                  ...
                  <p class="p-inline p-c-i"><i class="fas fa-clock"></i> {{$post->serie->duracion}}</p>
                  ...
                  <p class="text-center-mob"><a href="{{route('getserie',$post->pslug)}}" class="a-transparent-g">VER SERIE</a></p>
                  </section>
                </div>
