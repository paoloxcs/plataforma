  ...
  <section class="col-md-12 bg-gris pbm-0 s-cursos"> 
   ...
   <section class="col-md-10 mx-auto">
    ...
    <section class="col-md-10 mx-auto row pbm-0 mt-5 mt-2-mob">
      <h5 class="title-c font-weight">CAPACITACIÓN</h5>
      {{-- <section class="col-md-2 col-xs-12"> --}}
        ...
        {{-- @foreach ($categorias as $cat)
            <li class="">
                <a href="{{ route('videoCat', $cat->slug) }}" class=""
                    style="width: 80%;position: relative;display: inline-block">{{ $cat->nombrecategoria }}</a>
                <span class="badge badge-secondary float-right"
                    style="background: #24db37;float:right;margin-top: 1%">{{ $cat->countVideos() }}</span>
            </li>
        @endforeach --}}
        @foreach ($categorias as $cat)
            <li class="">
                <a href="{{ route('videoCat', $cat->slug) }}" class=""
                    style="width: 80%;position: relative;display: inline-block">{{ $cat->nombrecategoria }}</a>
                <span class="badge badge-secondary float-right"
                    style="background: #24db37;float:right;margin-top: 1%">{{ $cat->countVideosActive() }}</span>
            </li>
        @endforeach
        </ul>
      ...
      <section class="col-md-9 col-xs-12">
        ...
        <div class="col-md-12 row pbm-0 mt-5 mt-2-mob">
             @foreach($posts as $post)
               <section class="col-md-4 col-xs-12 card-curso card-curso-p mt-2-mob">
                <div class="img">
                  <a href="{{route('getvideo',$post->pslug)}}"><img src="{{asset('posts/'.$post->image)}}" width="100%"></a>
                   ...
                   {{-- TAG VIDEO --}}
                    ...
                    {{-- TAG GRATUITO --}}
                    @if ($post->acceso == '0')
                        <span class="s-disponible s-proximamente" style="right: 4%;left: unset; background: #2436db">GRATUITO</span>
                    @endif
