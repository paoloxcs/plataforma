...
<div class="container">
    <ol class="breadcrumb">
      <li class="breadcrumb-item active">
        <i class="fa fa-list-ol"></i> evaluacion : <a href="{{route('cursos')}}">{{$curso->titulo}}</a>
      </li>
    </ol>
              ...
              <table class="table table-condensed table-hover table-bordered">
                  ...
                  <tbody>
                    <tr>
		                ...
		                <td>{{$ev->fecha}}</td>
                    <td>
		                <a  href="{{route('evaluacion.editar',$ev->id)}}" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i> Editar</a>
		                <a onclick="return confirm('ADVERTENCIA:\n¿Seguro de eliminar el registro?')" href="{{route('evaluacion.delete',$ev->id)}}" class="btn btn-danger btn-sm"><i class="fa fa-trash" ></i> Eliminar</a>
                    <a href="{{route('cuestionario',$ev->id)}}" class="btn btn-warning btn-sm"><i class="fa fa-clipboard"></i> Cuestionario</a>
		                </td>
		              </tr>
                  </tbody>
                  @endforeach
    ...
    @include('panel.evaluacion.create')
</div>
    ...
    window.addEventListener('load',function(){
        document.getElementById("texto").addEventListener("keyup", () => {
            if((document.getElementById("texto").value.length)>=1)
                fetch(`{{route('evaluacion.buscador')}}?texto=${document.getElementById("texto").value}`,{ method:'get' })
