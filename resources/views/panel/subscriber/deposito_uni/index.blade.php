@extends('layouts.app')
@section('titulo', 'Suscriptores Universidad')
@section('extra_style')
    <style>
        .card {
            position: relative;
    ...
    <div class="container-fluid">
        <div class="panel panel-default">
            <div class="panel-heading">
                <div class="panel-title">Suscripcion Universidad</div>
            </div>
            <div class="panel-body">
                <div class="row">

                    <div class="col-xs-12 col-md-8" style="padding-bottom: 1rem">
                        <form action="{{ route('subscribers.premium_uni') }}" class="row col-10"
                            ...
                            @endif
                            @if (request('fecha_start'))
                                <input type="hidden" name="fechaInicial" value="{{ request('fecha_start') }}">
                            @endif
                            @if (request('fechaFinal'))
                                <input type="hidden" name="fechaFinal" value="{{ request('fechaFinal') }}">
                            @endif
                    ...
                    <div class="col-xs-12 col-md-4" style="display: flex; flex-direction: column">
                        <div style="display: flex">
                            ...
                            <span onclick="openNewModalFilter();" class="btn btn-warning"
                                ...
                                Filtros</span>
                            <a href="{{ route('subscribers.premium_uni') }}" class="btn btn-danger" title="Eliminar filtros"
                    ...
                    <table class="table table-condensed table-hover table-bordered">
                        ...
                        @forelse ($suscriptores_premium as $subs)
                            <tr>
                                ...
                                @php
                                    $nacimiento = new DateTime($subs->user->age);
                                    $ahora = new DateTime(date('Y-m-d'));
                                    $diferencia = $ahora->diff($nacimiento);
                                @endphp
                                <td>
                                    <div>
                                        <b>Profesión:</b>{{ $subs->user->profession ?? '--' }} <br>
                                        <b>Edad:</b>{{ $subs->user->age ? $diferencia->format('%y') . ' años' : '--' }}
                                        <br>
                                        <b>País:</b>{{ $subs->user->pais ?? '--' }} <br>
                                        <b>Teléfono:</b>{{ $subs->user->phone_number ?? '--' }} <br>
                                        <b>Universidad:</b>{{ $subs->user->universities()->first()->name }} <br>
                                    </div>
                                </td>
                                ...
                                @php
                                    $pagos = App\PagoUni::where('suscriptor_id', '=', $subs->id)
                                        ->orderBy('id', 'desc')
                                        ->first();
                                @endphp
                                ...
                                <td>
                                    ...
                                    {{-- Update Subscription --}}
                                    <button
                                        onclick='editSubscriber({{ $subs->user }},"{{ $subs->plan_id }}","{{ $subs->gestor_id }}","{{ $subs->suscription_end }}","{{ $subs->medio }}","{{ $subs->tipo }}")'
                ...
                @include('panel.subscriber.deposito_uni.new_filters')
...
@section('extra_scripts')
    <script>
        ...
        function editSubscriber(subs_user, subs_plan_id, subs_gestor_id, subs_suscription_end, subs_medio, subs_tipo) {
            ...
            //DATOS NUEVOS
            formEdit.profession.value = subs_user.profession;
            formEdit.cargo_user.value = subs_user.cargo_user;
            // formEdit.num_operacion.value = subs_user.num_operacion;
        ...
        function updateSubscriber(form) {
            ...
            if (validateForm(form)) {
                spinner.show();
                props.ruta = `/panel/subscribers/premium_uni/${form.user_id.value}`;
                let token = '{{ csrf_token() }}',
                    data = $(form).serialize();
                $.ajax({
                    url: props.ruta,
        ...
        function anulaSuscriptin(user_id) {
            if (confirm('¿Seguro de anular la suscripción?')) {
                props.ruta = `/panel/subscribers-premium_uni/${user_id}/destroy`;
        ...
        function exportNewExcel() {
            ...
            // let data = $("#form_filter").serialize();
            let url_download = `/panel/subscribers-premium_uni/download?${$parametros}`;
